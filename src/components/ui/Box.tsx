...
export default function Box({ name, value, disabled, text, handleClick } : { name : string , value: number | null, disabled: boolean, text:"X" | "O" | null, handleClick: (event: React.MouseEvent<HTMLInputElement>) => void }) {
    ...
    return (
        <div className="relative">
                <input type="button" name={ name } disabled={ disabled } className={ `${ value === null ? `bg-stone-50/80 hover:bg-stone-50/50 transition-all` : value === 1 ? `bg-sky-300` : `bg-teal-300` } w-16 h-16 sm:w-32 sm:h-32 border-none rounded-sm  backdrop-blur-xs shadow-xl cursor-pointer  ` } onClick={ handleClick } />
                { text &&
                <motion.div
                    initial={ { opacity: 0 } }
                    animate={ { opacity: 1 } }
